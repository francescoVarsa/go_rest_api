...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	// get the genres
	query = `select mg.id, mg.movie_id, mg.genre_id,
	 g.genre_name from movies_genres mg left join genres g on (g.id = mg.genre_id)
	 where mg.movie_id = $1`
...
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	for rows.Next() {
		...
		mg.genre_id,
		g.genre_name 
		from movies_genres mg left join genres g on (g.id = mg.genre_id)
		where mg.movie_id = $1`
